...
frc2::CommandPtr AutoGyroZeroWithPoseEstimate() {
  return SubDrivebase::GetInstance().ZeroRotation([] {
    ...
    frc2::CommandPtr Hoard_RightTrench(bool flip) {
      return frc2::cmd::Sequence(
        // STARTING POSITION: START_TRENCH_RIGHT (X 3.58m, Y 0.57m, heading 0 degrees)
        SubHood::GetInstance().ZeroHood(), cmd::ShootOnTheMove().WithTimeout(2_s),
        AutonomousDriveTo(frc::Pose2d{7.80_m, 1.04_m, 90_deg}, flip, 1.0, 20_cm),
        AutonomousDriveTo(frc::Pose2d{7.8_m, 6.0_m, 90_deg}, flip, 0.5, 20_cm, 5_deg)
          .AlongWith(cmd::ShootOnTheMove()));
    }

    frc2::CommandPtr Hoard_LeftTrench() {
      return Hoard_RightTrench(true);
    }
    ...
    frc2::CommandPtr NeutralOnePassAndOutpost_RightBump() {
      return frc2::cmd::Sequence(
        ...
        .AndThen(frc2::cmd::Sequence(
          ...
          AutonomousDriveTo(frc::Pose2d{7.80_m, 1.04_m, 90_deg}, false, 1.0, 20_cm),

          AutonomousDriveTo(frc::Pose2d{7.80_m, 3.29_m, 90_deg}, false, 0.5, 20_cm, 3_deg)
            .DeadlineFor(SubIntake::GetInstance().IntakeOn()),

          AutonomousDriveTo(frc::Pose2d{5.85_m, 2.64_m, 180_deg}, false, 1.0, 20_cm, 5_deg)
            .DeadlineFor(
              SubIntake::GetInstance().IntakeOn()),  // return to bump (continue intaking)

          SubDrivebase::GetInstance().DriveOverBump(frc::ChassisSpeeds{-3_mps, 0_mps, 0_tps},
            frc::Translation2d{3.5_m, 2.64_m})  // drive back over bump, reset position
          )
            .DeadlineFor(cmd::AimAtHub()))
        .AndThen(AutonomousDriveTo(frc::Pose2d{0.60_m, 0.67_m, 0_deg}, false, 1.0)
            .AlongWith(cmd::ShootOnTheMove()));
    }
    ...
    frc2::CommandPtr NeutralOnePassAndOutpost_RightTrench_NoBump() {
        return frc2::cmd::Sequence(
            ...
            AutonomousDriveTo(frc::Pose2d{5.70_m, 0.54_m, 270_deg}, false, 1.0, 20_cm, 5_deg)
                .DeadlineFor(SubIntake::GetInstance().IntakeOn()), //return to trench (continue intaking)

            AutonomousDriveTo(frc::Pose2d{3.50_m, 0.79_m, 270_deg}, false, 1.0, 20_cm, 5_deg)
        ).DeadlineFor(
            cmd::AimAtHub()
        )).AndThen(
            AutonomousDriveTo(frc::Pose2d{0.60_m, 0.67_m, 0_deg}, false, 1.0, 10_cm)
                .AlongWith(cmd::ShootOnTheMove())
        );
    }
    ...
    frc2::CommandPtr NeutralTwoPassToMid_RightTrench(bool flip) {
        return frc2::cmd::Sequence(
            AutoGyroZeroWithPoseEstimate(),

            // STARTING POSITION: START_TRENCH_RIGHT (X 3.58m, Y 0.57m, heading 90 degrees)
            AutonomousDriveTo(frc::Pose2d{5.80_m, 0.59_m, 100_deg}, flip, 1.0, 50_cm, 20_deg) //neutral side of trench
                .AlongWith(SubHood::GetInstance().ZeroHood()),
            ...
            frc2::cmd::Parallel(
                ...
                AutonomousDriveTo(frc::Pose2d{8.50_m, 1.07_m, 125_deg}, flip, 1.0, 50_cm, 20_deg), //first pass start
                AutonomousDriveTo(frc::Pose2d{8.50_m, 4.0_m, 125_deg}, flip, 0.7, 50_cm, 10_deg), //first pass end
                AutonomousDriveTo(frc::Pose2d{8.15_m, 4.0_m, 125_deg}, flip, 1.0, 30_cm, 30_deg), //cross centre line
                AutonomousDriveTo(frc::Pose2d{7.60_m, 0.59_m, 180_deg}, flip, 1.0, 50_cm, 20_deg), //transition
                AutonomousDriveTo(frc::Pose2d{3.60_m, 0.59_m, 180_deg}, flip, 1.0, 15_cm, 10_deg) //back to alliance zone
            )),
            ...
            SubIntake::GetInstance().IntakeOn().AlongWith(
                ...
                AutonomousDriveTo(frc::Pose2d{6.00_m, 0.59_m, 180_deg}, flip, 1.0, 40_cm, 20_deg), //neutral side of trench
                AutonomousDriveTo(frc::Pose2d{6.90_m, 0.77_m, 90_deg}, flip, 1.0, 50_cm, 25_deg), //second pass off-hub start
                AutonomousDriveTo(frc::Pose2d{6.40_m, 4.07_m, 110_deg}, flip, 1.0, 40_cm, 20_deg), //second pass off-hub end
                AutonomousDriveTo(frc::Pose2d{6.40_m, 4.07_m, 250_deg}, flip, 1.0, 40_cm, 20_deg), //turnaround
                AutonomousDriveTo(frc::Pose2d{6.90_m, 1.77_m, 250_deg}, flip, 1.0, 35_cm, 20_deg), //second pass off-hub start
                AutonomousDriveTo(frc::Pose2d{5.95_m, 0.59_m, 180_deg}, flip, 1.0, 40_cm, 20_deg), //transition to trench
                AutonomousDriveTo(frc::Pose2d{3.60_m, 0.59_m, 180_deg}, flip, 1.0, 20_cm, 10_deg) //drive under trench
            )),

            cmd::AutonomousShoot(4_s),
            AutonomousDriveTo(frc::Pose2d{6.10_m, 0.59_m, 180_deg}, flip, 1.0, 30_cm, 20_deg), //neutral side of trench
            AutonomousDriveTo(frc::Pose2d{7.80_m, 1.04_m, 90_deg}, flip, 1.0, 20_cm, 10_deg)
        );
    }

    frc2::CommandPtr NeutralTwoPassToMid_LeftTrench() {
      return NeutralTwoPassToMid_RightTrench(true);
    }

    frc2::CommandPtr ShortFirstPass_NeutralTwoPassToMid_RightTrench(bool flip) {
        return frc2::cmd::Sequence(
            AutoGyroZeroWithPoseEstimate(),
            ...
            frc2::cmd::Parallel(
                ...
                AutonomousDriveTo(frc::Pose2d{8.50_m, 1.07_m, 125_deg}, flip, 1.0, 50_cm, 20_deg), //first pass start
                AutonomousDriveTo(frc::Pose2d{8.15_m, 2.84_m, 125_deg}, flip, 0.7, 30_cm, 30_deg), //first pass end
                AutonomousDriveTo(frc::Pose2d{7.60_m, 0.59_m, 180_deg}, flip, 1.0, 50_cm, 20_deg), //transition
                AutonomousDriveTo(frc::Pose2d{3.60_m, 0.59_m, 180_deg}, flip, 1.0, 15_cm, 10_deg) //back to alliance zone
            )),
            ...
            SubIntake::GetInstance().IntakeOn().AlongWith(
                ...
                AutonomousDriveTo(frc::Pose2d{6.40_m, 4.07_m, 110_deg}, flip, 1.0, 40_cm, 20_deg), //second pass off-hub end
                AutonomousDriveTo(frc::Pose2d{6.40_m, 4.07_m, 250_deg}, flip, 1.0, 40_cm, 20_deg), //turnaround
                AutonomousDriveTo(frc::Pose2d{6.90_m, 0.77_m, 250_deg}, flip, 1.0, 35_cm, 20_deg), //second pass off-hub start
                AutonomousDriveTo(frc::Pose2d{5.95_m, 0.59_m, 180_deg}, flip, 1.0, 40_cm, 20_deg), //transition to trench
                AutonomousDriveTo(frc::Pose2d{3.60_m, 0.59_m, 180_deg}, flip, 1.0, 20_cm, 10_deg) //drive under trench
            )),
            ...
            SubIntake::GetInstance().IntakeOn().AlongWith(
                ...
                AutonomousDriveTo(frc::Pose2d{6.10_m, 0.59_m, 180_deg}, flip, 1.0, 50_cm, 20_deg), //neutral side of trench
                AutonomousDriveTo(frc::Pose2d{7.80_m, 1.04_m, 90_deg}, flip, 1.0, 20_cm, 10_deg)
            ))
        );
    }
    ...
    frc2::CommandPtr ShortFirstPass_NeutralTwoPassToMid_LeftTrench() {
      return ShortFirstPass_NeutralTwoPassToMid_RightTrench(true);
    }
